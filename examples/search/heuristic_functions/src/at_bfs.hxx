...
#include <algorithm>
#include <iostream>
#include "nested_bucket_queue.hxx"
...
public:
	...
	Node( State* s, int cost, Action_Idx action, Node<State>* parent )
	...
	int&			hn()		{ return m_h; }
	int			hn() const 	{ return m_h; }
	int&			gn()		{ return m_g; }
	int			gn() const 	{ return m_g; }
	int&			fn()		{ return m_f; }
	int			fn() const	{ return m_f; }
...
public:
	...
	typedef aptk::agnostic_int::Nested_Bucket_Queue<Search_Node*>          Open_Bucket;
	...
	m_exp_count(0), m_gen_count(0), m_pruned_B_count(0), m_dead_end_count(0), m_open_repl_count(0),
	  m_B( int_infty ), m_time_budget(infty), m_greedy(false), m_delay_eval(true) {
	...
	virtual ~AT_BFS_SQ_SH() {
		...
		while	(!m_open.empty() )
			//while	(!m_open_bucket.empty() )
		{
			Search_Node* n = m_open.pop();
			//Search_Node* n = m_open_bucket.pop();
			delete n;
		}
		m_closed.clear();
		m_open_hash.clear();
	...
	void	start() {
		...
		if(m_greedy)
			...
			m_root->fn() = m_root->hn() + m_root->gn();



		m_open.insert( m_root );
		//m_open_bucket.push( m_root->fn(), m_root->gn(), m_root );
		m_open_hash.put( m_root );
		inc_gen();
	}

	bool	find_solution( int& cost, std::vector<Action_Idx>& plan ) {
		m_t0 = time_used();
	...
	Search_Node* 		get_node() {
		...
		if(! m_open.empty() ) {
		//if(! m_open_bucket.empty() ) {
		next = m_open.pop();
		//next = m_open_bucket.pop();
	...
	void	 	open_node( Search_Node *n ) {
		...
		m_open.insert(n);
		//	m_open_bucket.push( n->fn(), n->gn(), n );
			m_open_hash.put(n);
		}
		inc_gen();
	}
	...
	virtual Search_Node*	 	do_search() {
		...
		while(head) {
			...
			if ( head->fn() > min_h ) {
				min_h = head->fn();
				std::cout << "\t Best f(n) = " << min_h << ", expanded: " << expanded() << ", generated: " << generated() << std::endl;
			}
...
protected:
	...
	void	extract_plan( Search_Node* s, Search_Node* t, std::vector<Action_Idx>& plan, int& cost ) {
		Search_Node *tmp = t;
		cost = 0.0f;
		while( tmp != s) {
